...
public class student {  
    ...
    //writing constructor:  
    /**  
     - name of the class and name of the constructors must be same     - here below in student constructor, we put compulsion on the user that when student object is created,       compulsory there must be name and roll number as arguments. this compulsion avoids the case from above notes where       every student has null name and 0 roll number. */    student(String name, int roll){  
        this.name = name;  
        this.roll = roll;  
    }  
    ...
    public static void main(String[] args) {  
        ...
        * as name property of the student class. i said current "akash" because in the context of constructors, this keyword is        * used to provide current context. in the current context in the above line, name is akash for temporary basis in constructor.        * when constructor's this.name = name line executes, this is where the "akash" will be mapped to the object's name variable        * so then s1.name will return "akash"        * */        student s2 = new student("samyak", 10);  
